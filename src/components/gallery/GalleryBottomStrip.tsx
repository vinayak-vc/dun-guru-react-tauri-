...
export type GalleryBottomStripProps = {
...
/**
 * Bottom thumbnail row — layout aligned with Figma export for Screen 3:
 * thumb frame (similar to Screen 5) at y=3487, tiles 194×194, gap ≈36, strip width 1114.
 * Selected item uses green ring per kiosk mock.
 */
export const GalleryBottomStrip: React.FC<GalleryBottomStripProps> = ({
  items,
  selectedIndex,
  onSelectIndex,
}) => {
  ...
  const safeSelectedIndex = Number.isFinite(selectedIndex) ? selectedIndex : 0;
  ...
  React.useLayoutEffect(() => {
    if (n <= 0) return;
    // Ensure we start in the middle copy so wrap-neighbors exist.
    const scroller = scrollerRef.current;
    if (scroller) {
      scroller.scrollLeft = n * STEP;
    }

    // Center the selected item. Retry next frame if refs aren't ready yet.
  ...
  const normalizeScrollWindow = () => {
    ...
    if (!el || n <= 0) return;
    // Keep the user near the middle copy so it feels infinite.
  ...
  const handleScrollPrev = () => {
    if (n <= 0) return;
    const nextIndex = (normalizedIndex - 1 + n) % n;
    onSelectIndex(nextIndex);
  };

  const handleScrollNext = () => {
    if (n <= 0) return;
    const nextIndex = (normalizedIndex + 1) % n;
    onSelectIndex(nextIndex);
  };
    ...
    >
      ...
      >
        {loopItems.map((item, virtualIndex) => {
          ...
          // Only highlight the middle-copy instance so selection never "duplicates".
          ...
          return (
            <button
              ...
              type="button"
              tabIndex={0}
              aria-label={item.title || `Image ${logicalIndex + 1}`}
              ...
              className={`relative h-[194px] w-[194px] shrink-0 overflow-hidden rounded-[24px] border-2 transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-white/55 ${
                selected
                  ? 'border-emerald-400 shadow-[0_0_0_4px_rgba(52,211,153,0.45)]'
